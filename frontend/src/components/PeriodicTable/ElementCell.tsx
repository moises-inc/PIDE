...
import { categoryLabel, formatValue, metalClassLabel, phaseLabel } from '../../utils/chemistry';
...
export function ElementCell({ element, heatValue, heatMin, heatMax, isSelected, isCompared, phase, dimmed, onSelect }: ElementCellProps) {
  ...
  const metalClass = element.metalClass ?? 'unknown';
  const metalText = metalClassLabel(element.metalClass, element.category);
  const categoryText = categoryLabel(element.category);

  return (
    <button
      className={`element-cell category-${categoryClass} metal-${metalClass} ${isSelected ? 'is-selected' : ''} ${isCompared ? 'is-compared' : ''} ${dimmed ? 'is-dimmed' : ''}`}
      style={style}
      type="button"
      onClick={() => onSelect(element.z)}
      aria-label={`${element.nameEs}, ${element.symbol}, número atómico ${element.z}. ${metalText} (${categoryText}).`}
      aria-pressed={isSelected}
      title={`${element.nameEs} · ${metalText} (${categoryText})`}
    >
